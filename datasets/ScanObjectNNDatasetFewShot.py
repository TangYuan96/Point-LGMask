...
class ScanObjectNNFewShot(Dataset):
    def __init__(self, config):
        ...
        print_log('Load processed data from %s...' % self.pickle_path, logger = 'ScanObjectNNFewShot')

        with open(self.pickle_path, 'rb') as f:
            self.dataset = pickle.load(f)[self.subset]

        print_log('The size of %s data is %d' % (split, len(self.dataset)), logger = 'ScanObjectNNFewShot')
