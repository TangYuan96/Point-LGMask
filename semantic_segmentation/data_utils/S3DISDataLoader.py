...
class S3DISDataset_ACT_used(Dataset):
    def __init__(self, split='train', data_root='trainval_fullarea', num_point=4096, test_area=5, block_size=1.0, sample_rate=1.0, transform=None):
        super().__init__()
        self.num_point = num_point
        self.block_size = block_size
...
if __name__ == '__main__':
    ...
    data_root = '/home/pc_msn_61/pc_msn/data/S3DIS/stanford_indoor3d/'
    ...
    TRAIN_DATASET = S3DISDataset_ACT_used(split='test', data_root=data_root, num_point=num_point, test_area=5)
    trainDataLoader = torch.utils.data.DataLoader(TRAIN_DATASET, batch_size=2, shuffle=True, num_workers=10, drop_last=True)

    for i, (points, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
        orign_t = 'test_origin_' + str(i) + "_.txt"
        np.savetxt(orign_t, points[0], fmt='%f')

        label_t = 'test_origin_label_' + str(i) + "_.txt"
        np.savetxt(label_t, target[0], fmt='%d')
        print("pre main arg")
        print(points.shape)

    test_aa = point_data.__getitem__(0)
    ...
    for idx in range(4):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            orign_t = 'test_origin_'+str(i)+"_.txt"
            np.savetxt(orign_t, input, fmt='%f')

            label_t = 'test_origin_label_' + str(i) + "_.txt"
            np.savetxt(label_t, target, fmt='%d')
            end = time.time()