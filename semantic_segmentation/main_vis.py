...
def parse_args():
    ...
    parser.add_argument('--root', type=str, default='../data/S3DIS/stanford_indoor3d/', help='data root')
    ...
    parser.add_argument('--vis_dir', type=str, default='./vis/test', help=' the dir for pre, gt, pc')
...
def main(args):
    ...
    '''HYPER PARAMETER'''
    experiment_dir = args.vis_dir
    os.makedirs(experiment_dir, exist_ok=True)
    os.makedirs(experiment_dir+'/npy', exist_ok=True)
    ...
    with torch.no_grad():
        ...
        for batch_idx in range(num_batches):
            ...
            ##  for poin-lgmask##
            # if tmp_iou < 0.75:
            #     print("no save [%d/%d] %s ..." % (batch_idx + 1, num_batches, scene_id[batch_idx]))
            #     continue
            np.save(experiment_dir+'/npy/point_LGMask_pre_l_'+str(batch_idx)+'_.npy', pred_label)
            np.save(experiment_dir+'/npy/gt_l_'+str(batch_idx)+'_.npy', whole_scene_label)
            np.save(experiment_dir+'/npy/pc_'+str(batch_idx)+'_.npy', whole_scene_data)
            continue
            ## for poin-lgmask##

            ### for transformer
            # np.save('experiment_dir+npy/pcvit_pre_l_'+str(batch_idx)+'_.npy', pred_label)
            # # np.save('experiment_dir+npy/gt_l_'+str(batch_idx)+'_.npy', whole_scene_label)
            # # np.save('experiment_dir+npy/pc_'+str(batch_idx)+'_.npy', whole_scene_data)
            # continue
            ### for transformer
