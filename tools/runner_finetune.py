...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        print_log('best_metrics_vote: %.4f' % (best_metrics_vote.acc), logger=logger)
        print_log('exp_name: ' + (str(args.experiment_path)), logger=logger)
...
def test_net(args, config):
    ...
    test(base_model, test_dataloader, args, config, logger=logger, turn_i=300)
...
def test(base_model, test_dataloader, args, config, logger = None, turn_i=10):
    ...
    with torch.no_grad():
        ...
        print_log('[TEST] standard_acc = %.4f' % standard_acc, logger=logger)
        ...
        print_log('[TEST] stand_acc = %.4f' % standard_acc, logger=logger)
