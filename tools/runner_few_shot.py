...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if args.way*args.shot < 32:
        config.total_bs=args.way*args.shot
    ...
    print_log('total_bs:%d !!!!' % (config.total_bs), logger=logger)

    config.dataset.train.others.fold = args.fold
    config.dataset.val.others.fold = args.fold

    print_log('way-%d shot-%d fold-%d start!!!!' % (args.way, args.shot, args.fold), logger=logger)
    # build dataset
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            if idx % 10 == 0:
                print_log(
                    '[Epoch %d/%d][Batch %d/%d] BatchTime = %.3f (s) DataTime = %.3f (s) Loss+Acc = %s lr = %.6f' %
        ...
        print_log('best_metrics: %.4f' % (best_metrics.acc), logger=logger)
...
def test_net(args, config):
    ...
    test(base_model, test_dataloader, args, config, logger=logger, turn_i=100)


def test(base_model, test_dataloader, args, config, logger=None, turn_i=10):
    base_model.eval()  # set model to eval mode
