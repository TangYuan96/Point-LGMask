...
## we use adamw and cosine scheduler
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            ...
            mean_acc = np.mean(np.array(total_correct_class) / (np.array(total_seen_class, dtype=np.float32) + 1e-6))
            log_string('[mAcc] eval point avg class acc: %f' % (mean_acc * 100.0))
            # log_string('[mAcc] eval point avg class acc: %f' % (
            #     np.mean(np.array(total_correct_class) / (np.array(total_seen_class, dtype=np.float32) + 1e-6)) * 100.0))
            ...
            if (mIoU >= 0.608) or (mean_acc >= 0.699):

                savepath = str(checkpoints_dir) + f'/model_epoch_{epoch}_IoU_{mIoU}_mean_Acc_{mean_acc}.pth'
            ...
            if mIoU >= best_iou:
                ...
                # savepath = str(checkpoints_dir) + f'/model_best.pth'
                log_string('Saving at %s' % savepath)
                state = {
                    'epoch': epoch,
                    'class_avg_iou': mIoU,
