...
def get_ptcloud_img(ptcloud,origPoints, view_roll=30, view_pitch=45):
    fig = plt.figure(figsize=(8, 8))

    x, z, y = ptcloud.transpose(1, 0)
    x_1, z_1, y_1 = origPoints.transpose(1, 0)
    ...
    ax.axis('off')
    # ax.axis('scaled')
    ax.view_init(view_roll,view_pitch)

    max, min = np.max(origPoints), np.min(origPoints)
    ax.set_xbound(min, max)
    ax.set_ybound(min, max)
    ax.set_zbound(min, max)

    # gist_earth
    ax.scatter(x, y, z, zdir='z',c=y, cmap="gist_earth")

    #Prevent deformation
    ax.scatter(x_1, y_1, z_1, zdir='z', alpha=0)
