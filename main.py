...
from tools import few_shot_run_net as finetune_few_shot
...
def main():
    ...
    # cp key files
    key_files_path = args.experiment_path+'/key_files'
    os.system('mkdir   %s' % (key_files_path))
    os.system('cp %s %s' % ('./main.py', key_files_path +'/'))
    os.system('cp -r %s %s' % ('./tools', key_files_path +'/'))
    os.system('cp -r %s %s' % ('./cfgs', key_files_path +'/'))
    os.system('cp %s %s' % ('./models/Point_LGMask.py', key_files_path +'/'))
    os.system('cp %s %s' % ('./utils/parser.py', key_files_path +'/'))
    ...
    if args.test:
        ...
        if args.few_shot:
            finetune_few_shot(args, config, train_writer, val_writer)
